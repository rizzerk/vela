...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'financial_data') {
    ...
    // chart data for selected year (show 12 months)
    ...
    // Get yearly data for rent and utilities from bills
    $chart_bill_rent_query = "SELECT MONTH(b.generated_at) as month, b.bill_type, SUM(b.amount) as total FROM BILL b JOIN LEASE l ON b.lease_id = l.lease_id WHERE b.status = 'paid' AND b.bill_type IN ('rent', 'utility') $date_condition_bill $property_condition GROUP BY MONTH(b.generated_at), b.bill_type";
    $chart_bill_rent_result = $conn->query($chart_bill_rent_query);
    if ($chart_bill_rent_result) {
        while ($row = $chart_bill_rent_result->fetch_assoc()) {
            $month_index = $row['month'] - 1;
            $chart_data[$row['bill_type']][$month_index] = (float)$row['total'];
        }
    }
    
    // to kuha yearly data for bills
    $chart_bill_query = "SELECT MONTH(b.generated_at) as month, b.bill_type, SUM(b.amount) as total FROM BILL b JOIN LEASE l ON b.lease_id = l.lease_id WHERE b.status = 'paid' AND b.bill_type IN ('penalty', 'other') $date_condition_bill $property_condition GROUP BY MONTH(b.generated_at), b.bill_type";
    ...
    $total_income = $rent_collected + $utilities + $penalties;
    $total_expenses = $other; // Only 'other' bills are actual expenses
...
// Yearly data for chart - all from BILL table for consistency
$yearly_bill_query = "
    SELECT 
...
$yearly_data = $yearly_bill_result ? $yearly_bill_result->fetch_all(MYSQLI_ASSOC) : [];
...
// Correct expense calculation - utilities are tenant payments (income), not expenses
$total_income = $yearly_collected + $yearly_utilities + $yearly_penalties;
$total_expenses = $yearly_other; // Only 'other' bills are actual expenses
$net_profit = $total_income - $total_expenses;
...
<body>
            ...
            <div class="financial-cards">
                ...
                <div class="financial-card income-card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="financial-metric">
                            <span class="metric-label">Utilities Paid</span>
                            <span class="metric-value">₱<?= number_format($yearly_utilities, 2) ?></span>
                        </div>
                        <div class="financial-metric">
                            <span class="metric-label">Penalties Collected</span>
    ...
    <script>
        ...
        function updateFinancialCards(data) {
            ...
            document.querySelector('.income-card .card-body .financial-metric:nth-child(3) .metric-value').textContent = 
            ...
            document.querySelector('.income-card .card-body .financial-metric.total .metric-value').textContent = 
                '₱' + parseFloat(data.total_income).toLocaleString('en-US', {minimumFractionDigits: 2});
