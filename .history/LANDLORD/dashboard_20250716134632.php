...
// Check if PAYMENT table exists and get data
if ($conn->query("SHOW TABLES LIKE 'PAYMENT'") && $conn->query("SHOW TABLES LIKE 'PAYMENT'")->num_rows > 0) {
    $monthly_rent_query = "SELECT COALESCE(SUM(amount), 0) as monthly_rent FROM PAYMENT WHERE payment_type = 'rent' AND DATE_FORMAT(payment_date, '%Y-%m') = '$current_month' AND status = 'completed'";
    $result = $conn->query($monthly_rent_query);
    $monthly_rent = $result && $result->num_rows > 0 ? $result->fetch_assoc()['monthly_rent'] : 0;
    
    $monthly_utilities_query = "SELECT COALESCE(SUM(amount), 0) as utilities FROM PAYMENT WHERE payment_type = 'utilities' AND DATE_FORMAT(payment_date, '%Y-%m') = '$current_month' AND status = 'completed'";
    $result = $conn->query($monthly_utilities_query);
    $monthly_utilities = $result && $result->num_rows > 0 ? $result->fetch_assoc()['utilities'] : 0;
    
    $yearly_rent_query = "SELECT COALESCE(SUM(amount), 0) as yearly_rent FROM PAYMENT WHERE payment_type = 'rent' AND YEAR(payment_date) = '$current_year' AND status = 'completed'";
    $result = $conn->query($yearly_rent_query);
    $yearly_rent = $result && $result->num_rows > 0 ? $result->fetch_assoc()['yearly_rent'] : 0;
    
    $yearly_utilities_query = "SELECT COALESCE(SUM(amount), 0) as yearly_utilities FROM PAYMENT WHERE payment_type = 'utilities' AND YEAR(payment_date) = '$current_year' AND status = 'completed'";
    $result = $conn->query($yearly_utilities_query);
    $yearly_utilities = $result && $result->num_rows > 0 ? $result->fetch_assoc()['yearly_utilities'] : 0;
} else {
    // Fallback to property rent if PAYMENT table doesn't exist
    $fallback_query = "SELECT COALESCE(SUM(monthly_rent), 0) as monthly_rent FROM PROPERTY WHERE status = 'occupied'";
    $result = $conn->query($fallback_query);
    $monthly_rent = $result && $result->num_rows > 0 ? $result->fetch_assoc()['monthly_rent'] : 0;
    $yearly_rent = $monthly_rent * 12;
}

// Check if MAINTENANCE_REQUEST table exists and get data
if ($conn->query("SHOW TABLES LIKE 'MAINTENANCE_REQUEST'") && $conn->query("SHOW TABLES LIKE 'MAINTENANCE_REQUEST'")->num_rows > 0) {
    $monthly_maintenance_query = "SELECT COALESCE(SUM(cost), 0) as maintenance FROM MAINTENANCE_REQUEST WHERE status = 'completed' AND DATE_FORMAT(completion_date, '%Y-%m') = '$current_month'";
    $result = $conn->query($monthly_maintenance_query);
    $monthly_maintenance = $result && $result->num_rows > 0 ? $result->fetch_assoc()['maintenance'] : 0;
    
    $yearly_maintenance_query = "SELECT COALESCE(SUM(cost), 0) as yearly_maintenance FROM MAINTENANCE_REQUEST WHERE status = 'completed' AND YEAR(completion_date) = '$current_year'";
    $result = $conn->query($yearly_maintenance_query);
    $yearly_maintenance = $result && $result->num_rows > 0 ? $result->fetch_assoc()['yearly_maintenance'] : 0;
}
...
for ($i = 11; $i >= 0; $i--) {
    ...
    $rent_query = "SELECT COALESCE(SUM(monthly_rent), 0) as rent FROM PROPERTY WHERE status = 'occupied'";
    $util_query = "SELECT 0 as utilities";
    $maint_query = "SELECT 0 as maintenance";
    
    // Try to get real data if tables exist
    if ($conn->query("SHOW TABLES LIKE 'PAYMENT'") && $conn->query("SHOW TABLES LIKE 'PAYMENT'")->num_rows > 0) {
        $rent_query = "SELECT COALESCE(SUM(amount), 0) as rent FROM PAYMENT WHERE payment_type = 'rent' AND DATE_FORMAT(payment_date, '%Y-%m') = '$month' AND status = 'completed'";
        $util_query = "SELECT COALESCE(SUM(amount), 0) as utilities FROM PAYMENT WHERE payment_type = 'utilities' AND DATE_FORMAT(payment_date, '%Y-%m') = '$month' AND status = 'completed'";
    }
    
    if ($conn->query("SHOW TABLES LIKE 'MAINTENANCE_REQUEST'") && $conn->query("SHOW TABLES LIKE 'MAINTENANCE_REQUEST'")->num_rows > 0) {
        $maint_query = "SELECT COALESCE(SUM(cost), 0) as maintenance FROM MAINTENANCE_REQUEST WHERE status = 'completed' AND DATE_FORMAT(completion_date, '%Y-%m') = '$month'";
    }
    
    $rent_result = $conn->query($rent_query);
    $util_result = $conn->query($util_query);
    $maint_result = $conn->query($maint_query);
    
    $monthly_data[] = [
        'rent' => $rent_result && $rent_result->num_rows > 0 ? $rent_result->fetch_assoc()['rent'] : 0,
        'utilities' => $util_result && $util_result->num_rows > 0 ? $util_result->fetch_assoc()['utilities'] : 0,
        'maintenance' => $maint_result && $maint_result->num_rows > 0 ? $maint_result->fetch_assoc()['maintenance'] : 0
    ];
}
    ...
    <div class="modal" id="announcementModal">
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <input type="hidden" name="visible_to" value="tenant">
        ...
        function createYearlyChart() {
            ...
            yearlyChart = new Chart(yearlyCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Rent Income',
                        data: [<?= implode(', ', array_column($monthly_data, 'rent')) ?>],
                        borderColor: '#10b981',
                        ...
                        label: 'Utilities',
                        data: [<?= implode(', ', array_column($monthly_data, 'utilities')) ?>],
                        borderColor: '#f59e0b',
                        ...
                        data: [<?= implode(', ', array_column($monthly_data, 'maintenance')) ?>],
                        borderColor: '#ef4444',
