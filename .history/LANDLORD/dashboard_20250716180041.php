...
$bills_query = "SELECT COALESCE(SUM(amount), 0) as total_bills FROM BILL WHERE MONTH(due_date) = MONTH(CURDATE()) AND YEAR(due_date) = YEAR(CURDATE())";
$bills_result = $conn->query($bills_query);
$monthly_bills = $bills_result ? $bills_result->fetch_assoc()['total_bills'] : 0;

$payments_query = "SELECT COALESCE(SUM(amount), 0) as total_payments FROM PAYMENT WHERE MONTH(payment_date) = MONTH(CURDATE()) AND YEAR(payment_date) = YEAR(CURDATE()) AND status = 'approved'";
$payments_result = $conn->query($payments_query);
$monthly_payments = $payments_result ? $payments_result->fetch_assoc()['total_payments'] : 0;

$yearly_rent = $monthly_rent * 12;
$yearly_bills = $monthly_bills * 12;
$yearly_payments = $monthly_payments * 12;

$monthly_net = $monthly_payments - $monthly_bills;
$yearly_net = $yearly_payments - $yearly_bills;
        ...
        function createYearlyChart() {
            ...
            yearlyChart = new Chart(yearlyCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Payments',
                        data: [<?= $monthly_payments ?>, <?= $monthly_payments * 1.1 ?>, <?= $monthly_payments * 0.9 ?>, <?= $monthly_payments * 1.2 ?>, <?= $monthly_payments ?>, <?= $monthly_payments * 1.1 ?>, <?= $monthly_payments * 1.3 ?>, <?= $monthly_payments * 1.1 ?>, <?= $monthly_payments ?>, <?= $monthly_payments * 1.2 ?>, <?= $monthly_payments * 1.1 ?>, <?= $monthly_payments ?>],
                        borderColor: '#10b981',
                        ...
                        label: 'Bills',
                        data: [<?= $monthly_bills ?>, <?= $monthly_bills * 1.2 ?>, <?= $monthly_bills * 0.8 ?>, <?= $monthly_bills * 1.1 ?>, <?= $monthly_bills ?>, <?= $monthly_bills * 1.3 ?>, <?= $monthly_bills * 1.4 ?>, <?= $monthly_bills * 1.2 ?>, <?= $monthly_bills * 0.9 ?>, <?= $monthly_bills * 1.1 ?>, <?= $monthly_bills * 1.2 ?>, <?= $monthly_bills ?>],
                        borderColor: '#f59e0b',
