...
if (isset($_GET['ajax']) && $_GET['ajax'] === 'financial_data') {
    ...
    // Get expected rent from active leases (monthly rent * 12)
    ...
    // Get actual rent collected from tenant payments
    $rent_query = "SELECT COALESCE(SUM(p.amount_paid), 0) as collected 
                   FROM PAYMENT p 
                   JOIN BILL b ON p.bill_id = b.bill_id 
                   JOIN LEASE l ON b.lease_id = l.lease_id 
                   WHERE p.status = 'verified' 
                   AND b.bill_type = 'rent' 
                   $date_condition_payment $property_condition";
    $rent_result = $conn->query($rent_query);
    $rent_collected = $rent_result ? $rent_result->fetch_assoc()['collected'] : 0;
    
    // Get utility payments from tenants
    $utilities_query = "SELECT COALESCE(SUM(p.amount_paid), 0) as utilities 
                       FROM PAYMENT p 
                       JOIN BILL b ON p.bill_id = b.bill_id 
                       JOIN LEASE l ON b.lease_id = l.lease_id 
                       WHERE p.status = 'verified' 
    ...
    // Get penalty payments from tenants
    $penalties_query = "SELECT COALESCE(SUM(p.amount_paid), 0) as penalties 
                       FROM PAYMENT p 
                       JOIN BILL b ON p.bill_id = b.bill_id 
                       JOIN LEASE l ON b.lease_id = l.lease_id 
                       WHERE p.status = 'verified' 
    ...
    // Get other payments from tenants
    $other_query = "SELECT COALESCE(SUM(p.amount_paid), 0) as other 
                   FROM PAYMENT p 
                   JOIN BILL b ON p.bill_id = b.bill_id 
                   JOIN LEASE l ON b.lease_id = l.lease_id 
                   WHERE p.status = 'verified' 
    ...
    // Get monthly payment data from tenant payments
    $chart_payment_query = "SELECT MONTH(p.submitted_at) as month, b.bill_type, SUM(p.amount_paid) as total 
                           FROM PAYMENT p 
                           JOIN BILL b ON p.bill_id = b.bill_id 
                           JOIN LEASE l ON b.lease_id = l.lease_id 
                           WHERE p.status = 'verified' 
    ...
    $chart_result = $conn->query($chart_payment_query);
    if ($chart_result) {
    ...
    $total_income = $rent_collected; // Only rent is income
    $total_expenses = $utilities + $penalties + $other; // These are tenant payments for expenses
    $collection_rate = $expected_rent > 0 ? ($rent_collected / $expected_rent) * 100 : 0;
    $net_profit = $total_income - $total_expenses; // Income minus what tenants paid for expenses
...
// Expected yearly rent from active leases
...
// Actual rent collected from tenant payments this year
...
// Utility payments from tenants this year
...
// Penalty payments from tenants this year
$penalty_expenses_query = "
    SELECT COALESCE(SUM(p.amount_paid), 0) as penalty_expenses
    FROM PAYMENT p 
    JOIN BILL b ON p.bill_id = b.bill_id
    WHERE p.status = 'verified' 
    AND b.bill_type = 'penalty'
...
// Other payments from tenants this year
...
// Get monthly payment data from tenants for chart
...
$yearly_data = $yearly_payment_result ? $yearly_payment_result->fetch_all(MYSQLI_ASSOC) : [];
