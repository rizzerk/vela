...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_status'])) {
  ...
  try {
      ...
      $appStmt = $conn->prepare("SELECT a.*, p.title, u.name as applicant_name, u.email, p.monthly_rent 
                               FROM APPLICATIONS a
                               JOIN PROPERTY p ON a.property_id = p.property_id
                               JOIN USERS u ON a.applicant_id = u.user_id
      ...
      if ($status == 'approved') {
          ...
          sendApprovalEmail(
              ...
              $application['email'],
              $application['title'],
              $application['monthly_rent'] // Pass rent amount for reference
          );
          
      } elseif ($status == 'deposit_paid') {
        // Should be just updating deposit_paid flag, not changing status
        $depositStmt = $conn->prepare("UPDATE APPLICATIONS SET deposit_paid = TRUE WHERE application_id = ?");
        $depositStmt->bind_param("i", $application_id);
        $depositStmt->execute();
        $depositStmt->close();
        
        // Rest of your lease creation code...
          ...
          $billStmt = $conn->prepare("INSERT INTO BILL 
              (lease_id, amount, due_date, status, description, 
               billing_period_start, bill_type) 
              VALUES (?, ?, ?, ?, ?, ?, ?)");
