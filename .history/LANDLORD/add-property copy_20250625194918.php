...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    mysqli_stmt_bind_param($stmt, "ssssd", $title, $address, $status, $description, $monthly_rent);
        ...
        if (!empty($_FILES['photos']['name'][0])) {
            ...
            foreach ($_FILES['photos']['tmp_name'] as $key => $tmp_name) {
                ...
                if (move_uploaded_file($tmp_name, $file_path)) {
                    $query = "INSERT INTO PROPERTY_PHOTO (property_id, file_path) VALUES (?, ?)";
                    $stmt = mysqli_prepare($conn, $query);
                    mysqli_stmt_bind_param($stmt, "is", $property_id, $file_path);
