            ...
            <div class="financial-cards">
                ...
                <div class="financial-card expenses-card">
                    ...
                    <div class="card-body">
                        ...
                        <?php 
                        $monthly_penalties_query = "SELECT COALESCE(SUM(p.amount_paid), 0) as penalties FROM PAYMENT p JOIN BILL b ON p.bill_id = b.bill_id WHERE p.status = 'verified' AND b.bill_type = 'penalty' AND MONTH(p.submitted_at) = MONTH(NOW()) AND YEAR(p.submitted_at) = YEAR(NOW())";
                        $penalties_result = $conn->query($monthly_penalties_query);
                        $monthly_penalties = $penalties_result ? $penalties_result->fetch_assoc()['penalties'] : 0;
                        
                        $monthly_other_query = "SELECT COALESCE(SUM(p.amount_paid), 0) as other FROM PAYMENT p JOIN BILL b ON p.bill_id = b.bill_id WHERE p.status = 'verified' AND b.bill_type = 'other' AND MONTH(p.submitted_at) = MONTH(NOW()) AND YEAR(p.submitted_at) = YEAR(NOW())";
                        $other_result = $conn->query($monthly_other_query);
                        $monthly_other = $other_result ? $other_result->fetch_assoc()['other'] : 0;
                        ?>
                        <div class="financial-metric">
