...
// Get actual rent payments for current month
$monthly_rent_query = "SELECT COALESCE(SUM(amount), 0) as monthly_rent 
                      FROM PAYMENT 
...
// Get actual utility payments for current month
$monthly_utilities_query = "SELECT COALESCE(SUM(amount), 0) as utilities 
                           FROM PAYMENT 
                           WHERE payment_type = 'utilities' 
                           AND DATE_FORMAT(payment_date, '%Y-%m') = '$current_month'
                           AND status = 'completed'";
$result = $conn->query($monthly_utilities_query);
$monthly_utilities = $result ? $result->fetch_assoc()['utilities'] : 0;

// Get actual maintenance expenses for current month
$monthly_maintenance_query = "SELECT COALESCE(SUM(cost), 0) as maintenance 
                             FROM MAINTENANCE_REQUEST 
                             WHERE status = 'completed' 
                             AND DATE_FORMAT(completion_date, '%Y-%m') = '$current_month'";
$result = $conn->query($monthly_maintenance_query);
$monthly_maintenance = $result ? $result->fetch_assoc()['maintenance'] : 0;

// Get yearly totals
$yearly_rent_query = "SELECT COALESCE(SUM(amount), 0) as yearly_rent 
                     FROM PAYMENT 
                     WHERE payment_type = 'rent' 
                     AND YEAR(payment_date) = '$current_year'
                     AND status = 'completed'";
$result = $conn->query($yearly_rent_query);
$yearly_rent = $result ? $result->fetch_assoc()['yearly_rent'] : 0;

$yearly_utilities_query = "SELECT COALESCE(SUM(amount), 0) as yearly_utilities 
                          FROM PAYMENT 
                          WHERE payment_type = 'utilities' 
                          AND YEAR(payment_date) = '$current_year'
                          AND status = 'completed'";
$result = $conn->query($yearly_utilities_query);
$yearly_utilities = $result ? $result->fetch_assoc()['yearly_utilities'] : 0;

$yearly_maintenance_query = "SELECT COALESCE(SUM(cost), 0) as yearly_maintenance 
                            FROM MAINTENANCE_REQUEST 
                            WHERE status = 'completed' 
                            AND YEAR(completion_date) = '$current_year'";
$result = $conn->query($yearly_maintenance_query);
$yearly_maintenance = $result ? $result->fetch_assoc()['yearly_maintenance'] : 0;
...
for ($i = 11; $i >= 0; $i--) {
    ...
    $monthly_data[] = [
        'rent' => $rent_result ? $rent_result->fetch_assoc()['rent'] : 0,
        'utilities' => $util_result ? $util_result->fetch_assoc()['utilities'] : 0,
        'maintenance' => $maint_result ? $maint_result->fetch_assoc()['maintenance'] : 0
    ];
}
