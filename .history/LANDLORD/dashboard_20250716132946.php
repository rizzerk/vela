...
// Financial Data - Updated to fetch actual payment data
$financial_query = "SELECT 
    MONTH(payment_date) as month,
    SUM(amount) as total_rent,
    SUM(CASE WHEN payment_type = 'utility' THEN amount ELSE 0 END) as total_utilities
FROM PAYMENTS
WHERE 
    YEAR(payment_date) = YEAR(CURDATE()) AND
    status = 'completed'
GROUP BY MONTH(payment_date)";

$financial_result = $conn->query($financial_query);
$monthly_data = array_fill(1, 12, ['rent' => 0, 'utilities' => 0]);

if ($financial_result) {
    while ($row = $financial_result->fetch_assoc()) {
        $month = (int)$row['month'];
        $monthly_data[$month]['rent'] = (float)$row['total_rent'];
        $monthly_data[$month]['utilities'] = (float)$row['total_utilities'];
    }
}
...
$utilities_data = [];
for ($i = 1; $i <= 12; $i++) {
    $rent_data[] = $monthly_data[$i]['rent'];
    $utilities_data[] = $monthly_data[$i]['utilities'];
}

// Get totals for display
$total_rent = array_sum(array_column($monthly_data, 'rent'));
$total_utilities = array_sum(array_column($monthly_data, 'utilities'));
    ...
    <div class="main-content">
        ...
        <div class="financial-grid">
            <div class="card">
                ...
                <div class="metric">
                    <span class="metric-label">Total Rent Collected</span>
                    <span class="metric-value">₱<?= number_format($total_rent, 2) ?></span>
                </div>
                <div class="metric">
                    <span class="metric-label">Total Utilities Collected</span>
                    <span class="metric-value">₱<?= number_format($total_utilities, 2) ?></span>
                </div>
