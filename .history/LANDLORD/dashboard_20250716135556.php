...
// Get actual utility expenses this month
...
// Get actual maintenance costs this month
...
$monthly_net = $monthly_rent - $monthly_utilities - $monthly_maintenance;
$yearly_net = $yearly_rent - $yearly_utilities - $yearly_maintenance;
...
for ($i = 11; $i >= 0; $i--) {
    ...
    $rent_query = "SELECT COALESCE(SUM(amount), 0) as rent FROM RENT_PAYMENT WHERE DATE_FORMAT(payment_date, '%Y-%m') = '$month' AND payment_status = 'paid'";
    $util_query = "SELECT COALESCE(SUM(amount), 0) as utilities FROM EXPENSE WHERE expense_type = 'utilities' AND DATE_FORMAT(expense_date, '%Y-%m') = '$month'";
