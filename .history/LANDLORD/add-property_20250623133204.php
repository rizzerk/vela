...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($address) || empty($status) || $monthly_rent <= 0) {
        ...
        try {
            ...
            // Insert property into database using prepared statement
            $stmt = $conn->prepare("INSERT INTO PROPERTY (title, address, status, description, monthly_rent) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssd", $title, $address, $status, $description, $monthly_rent);
            ...
            if ($stmt->execute()) {
                ...
                if (isset($_FILES['photos']) && !empty($_FILES['photos']['name'][0])) {
                    ...
                    $photo_stmt = $conn->prepare("INSERT INTO PROPERTY_PHOTO (property_id, file_path) VALUES (?, ?)");
                    
                    foreach ($_FILES['photos']['tmp_name'] as $key => $tmp_name) {
                        if (!empty($tmp_name) && $_FILES['photos']['error'][$key] === UPLOAD_ERR_OK) {
                            // Validate file type
                            ...
                            if (in_array($file_type, $allowed_types)) {
                                ...
                                $file_name = time() . '_' . $key . '_' . uniqid() . '.' . $file_extension;
                                $file_path = $upload_dir . $file_name;
                                
                                if (move_uploaded_file($tmp_name, $file_path)) {
                                    $relative_path = 'uploads/properties/' . $file_name;
                                    $photo_stmt->bind_param("is", $property_id, $relative_path);
                                    $photo_stmt->execute();
                                }
                            }
                        }
