...
$properties = [];
if ($conn) {
    $stmt = $conn->prepare("SELECT p.property_id, p.title, p.address, p.monthly_rent, p.description, pp.file_path FROM PROPERTY p LEFT JOIN PROPERTY_PHOTO pp ON p.property_id = pp.property_id WHERE p.status = 'vacant' GROUP BY p.property_id ORDER BY p.property_id DESC");
    if ($stmt) {
        $stmt->execute();
