...
if ($lease) {
    // Try different possible announcement table structures
    $announcementQuery = "";
    $announcementStmt = null;
    
    // First try with ANNOUNCEMENT table
    try {
        $announcementQuery = "SELECT title, message, created_at FROM ANNOUNCEMENT 
                             WHERE property_id = ? 
                             ORDER BY created_at DESC LIMIT 3";
        $announcementStmt = $conn->prepare($announcementQuery);
        $announcementStmt->bind_param("i", $lease['property_id']);
        $announcementStmt->execute();
        $announcementResult = $announcementStmt->get_result();
    } catch (Exception $e) {
        // Try with NOTICE table if ANNOUNCEMENT doesn't exist
        try {
            $announcementQuery = "SELECT title, message, created_at FROM NOTICE 
                                 WHERE property_id = ? 
                                 ORDER BY created_at DESC LIMIT 3";
            $announcementStmt = $conn->prepare($announcementQuery);
            $announcementStmt->bind_param("i", $lease['property_id']);
            ...
            // Try without property_id filter
            try {
                $announcementQuery = "SELECT title, message, created_at FROM ANNOUNCEMENT 
                                     ORDER BY created_at DESC LIMIT 3";
                $announcementStmt = $conn->prepare($announcementQuery);
                ...
                $debug_info .= "Error: " . $e3->getMessage() . ", ";
        ...
        <?php if (!empty($announcements)): ?>
            <?php foreach ($announcements as $announcement): ?>
                <div class="notice-section">
                    ...
                    <p class="notice-text"><?= htmlspecialchars($announcement['message']) ?></p>
