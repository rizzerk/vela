...
if ($lease) {
    // Get lease start/end dates (handle missing columns gracefully)
    try {
        $leaseEventQuery = "SELECT lease_id FROM LEASE WHERE lease_id = ?";
        $leaseEventStmt = $conn->prepare($leaseEventQuery);
        $leaseEventStmt->bind_param("i", $lease['lease_id']);
        $leaseEventStmt->execute();
        $leaseEvents = ['start' => null, 'end' => null];
    } catch (Exception $e) {
        $leaseEvents = ['start' => null, 'end' => null];
    }

    // Get maintenance requests (handle missing table gracefully)
    try {
        $maintenanceQuery = "SELECT created_at as request_date FROM MAINTENANCE_REQUEST WHERE property_id = ? ORDER BY created_at DESC LIMIT 10";
    ...
    // Get payment events (handle gracefully)
    try {
        $paymentQuery = "SELECT p.created_at as payment_date, p.status FROM PAYMENT p 
        ...
        <div class="dashboard-grid">
            <div class="sidebar-content">
                <div class="calendar-section">
                    ...
                    <div class="calendar-legend" style="margin-top: 1rem; padding: 1rem; background: #f8fafc; border-radius: 8px;">
                        <div style="font-size: 0.85rem; font-weight: 600; color: #64748b; margin-bottom: 0.5rem;">Legend:</div>
                        <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 0.5rem; font-size: 0.75rem;">
                            <div style="display: flex; align-items: center; gap: 0.5rem;"><div style="width: 12px; height: 12px; background: #fef3c7; border-radius: 2px;"></div>Bills Due</div>
                            <div style="display: flex; align-items: center; gap: 0.5rem;"><div style="width: 12px; height: 12px; background: #dcfce7; border-radius: 2px;"></div>Lease Start</div>
                            <div style="display: flex; align-items: center; gap: 0.5rem;"><div style="width: 12px; height: 12px; background: #fecaca; border-radius: 2px;"></div>Lease End</div>
                            <div style="display: flex; align-items: center; gap: 0.5rem;"><div style="width: 12px; height: 12px; background: #e0e7ff; border-radius: 2px;"></div>Maintenance</div>
                            <div style="display: flex; align-items: center; gap: 0.5rem;"><div style="width: 12px; height: 12px; background: #f3e8ff; border-radius: 2px;"></div>Announcements</div>
                            <div style="display: flex; align-items: center; gap: 0.5rem;"><div style="width: 12px; height: 12px; background: #dbeafe; border-radius: 2px;"></div>Pending Payment</div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function renderCalendar() {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                const isLeaseStart = leaseEvents && leaseEvents.start === dateStr;
                const isLeaseEnd = leaseEvents && leaseEvents.end === dateStr;
