...
} elseif ($sort_by === 'priority_low') {
    $announcementQuery = "SELECT title, content, priority, created_at FROM ANNOUNCEMENT 
                         WHERE visible_to IN ('tenant', 'all') 
                         ORDER BY FIELD(priority, 'low', 'medium', 'high'), created_at DESC";
} else {
    $announcementQuery = "SELECT title, content, priority, created_at FROM ANNOUNCEMENT 
                         WHERE visible_to IN ('tenant') AND active = 1
...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="sort-section">
            <label for="sort" style="color: #64748b; font-weight: 500; margin-right: 10px;">Sort by:</label>
            <select id="sort" onchange="window.location.href='?sort=' + this.value" style="padding: 0.5rem; border: 1px solid #e2e8f0; border-radius: 8px; background: white;">
                ...
                <option value="priority_low" <?= $sort_by === 'priority_low' ? 'selected' : '' ?>>Priority (Low to High)</option>
            </select>
        </div>
