...
try {
    $maintenanceQuery = "SELECT mr.request_id, mr.created_at, mr.status 
                         FROM MAINTENANCE_REQUEST mr 
                         WHERE mr.property_id = ? 
                         ORDER BY mr.created_at DESC LIMIT 10";
    $maintenanceStmt = $conn->prepare($maintenanceQuery);
    $maintenanceStmt->bind_param("i", $lease['property_id']);
    ...
    // Get payment events
    try {
        $paymentQuery = "SELECT p.created_at, p.status FROM PAYMENT p JOIN BILL b ON p.bill_id = b.bill_id WHERE b.lease_id = ? AND p.status IN ('rejected', 'pending')";
        $paymentStmt = $conn->prepare($paymentQuery);
        $paymentStmt->bind_param("i", $lease['lease_id']);
        $paymentStmt->execute();
        $paymentResult = $paymentStmt->get_result();
        while ($row = $paymentResult->fetch_assoc()) {
            $paymentEvents[] = ['date' => $row['created_at'], 'status' => $row['status']];
        }
    } catch (Exception $e) {}
...
<head>
    ...
    <style>
        * {
        ...
        .event-bill { background: #fef3c7; color: #92400e; }
        .event-announcement { background: #ecfccb; color: #365314; }
        .event-maintenance { background: #ede9fe; color: #5b21b6; }
        .event-overdue { background: #fee2e2; color: #991b1b; }
        .event-pending { background: #dbeafe; color: #1e40af; }
        ...
        .event-lease { background: #dbeafe; color:rgb(39, 218, 41); }
        .event-overdue { background: #fee2e2; color: #991b1b; }
        .event-pending { background: #dbeafe; color:rgb(239, 222, 70); }
        .event-rejected { background: #fee2e2; color:rgb(222, 162, 10); }
        .event-maintenance { background: #ede9fe; color: #5b21b6; }
    </style>
</head>
...
<script>
    ...
    const events = {
        ...
        maintenance: <?= json_encode(isset($maintenanceEvents) ? $maintenanceEvents : []) ?>,
        payments: <?= json_encode($paymentEvents ?? []) ?>
    };
    ...
    function renderCalendar() {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const dayEvents = {
                leaseStart: events.lease.start && events.lease.start.startsWith(dateStr),
                leaseEnd: events.lease.end && events.lease.end.startsWith(dateStr),
                bills: events.bills.filter(bill => bill.date.startsWith(dateStr)),
                announcements: events.announcements.filter(ann => ann.date.startsWith(dateStr)),
                maintenance: events.maintenance.filter(maint => maint.date.startsWith(dateStr)),
                payments: events.payments.filter(payment => payment.date.startsWith(dateStr))
            };
            ...
            const hasEvents = dayEvents.leaseStart || dayEvents.leaseEnd || 
                             dayEvents.bills.length > 0 || 
                             dayEvents.announcements.length > 0 || 
                             dayEvents.maintenance.length > 0 ||
            ...
            // Lease events
            if (dayEvents.leaseStart) {
                eventHTML += '<div class="calendar-event event-lease">Lease Start</div>';
            }
            if (dayEvents.leaseEnd) {
                eventHTML += '<div class="calendar-event event-lease">Lease End</div>';
            }
            
            // Bill events
            dayEvents.bills.forEach(bill => {
                let statusClass = '';
                let statusText = '';
                let isOverdue = false;
                
                // Check if bill is overdue
                if (bill.status === 'unpaid' && dateObj < today) {
                    isOverdue = true;
                }
                
                if (isOverdue) {
                    statusClass = 'event-overdue';
                    statusText = 'Overdue: ₱' + parseFloat(bill.amount).toFixed(2);
                } else if (bill.status === 'unpaid') {
                    statusClass = 'event-bill';
                    statusText = 'Due: ₱' + parseFloat(bill.amount).toFixed(2);
                    ...
                    statusClass = 'event-paid';
                    statusText = 'Paid: ₱' + parseFloat(bill.amount).toFixed(2);
                }
                
                if (statusClass) {
                    eventHTML += `<div class="calendar-event ${statusClass}" title="${bill.description || 'Bill'}">${statusText}</div>`;
                }
            });
            
            // Payment events (separate from bills)
            dayEvents.payments.forEach(payment => {
                if (payment.status === 'pending') {
                    eventHTML += '<div class="calendar-event event-pending">Payment Pending</div>';
                } else if (payment.status === 'rejected') {
                    eventHTML += '<div class="calendar-event event-rejected">Payment Rejected</div>';
                }
            });
            
            // Announcements
            dayEvents.announcements.forEach(announcement => {
                eventHTML += `<div class="calendar-event event-announcement" title="${announcement.title}: ${announcement.content}">
                                ${announcement.priority} Announcement
                              </div>`;
            });
            
            // Maintenance requests
            dayEvents.maintenance.forEach(request => {
                let statusClass = '';
                if (request.status === 'completed') {
                    statusClass = 'event-maintenance-completed';
                } else if (request.status === 'in_progress') {
                    statusClass = 'event-maintenance-progress';
                } else {
                    statusClass = 'event-maintenance';
                }
                
                eventHTML += `<div class="calendar-event ${statusClass}">
                                Maintenance (${request.status.replace('_', ' ')})
                              </div>`;
            });
    ...
    function showDayEvents(dateStr) {
        ...
        let eventDetailsHTML = `
            <div class="event-modal">
                <div class="event-modal-header">
                    ...
                    <button onclick="closeEventModal()" class="close-modal">&times;</button>
                </div>
        ...
        if (dayEvents.leaseStart) {
            eventDetailsHTML += `
                <div class="event-detail lease-event">
                    <h4><i class="fas fa-file-contract"></i> Lease Start Date</h4>
                    <p>Your lease agreement starts on this date.</p>
                </div>
            `;
        }
        ...
        if (dayEvents.leaseEnd) {
            eventDetailsHTML += `
                <div class="event-detail lease-event">
                    <h4><i class="fas fa-file-contract"></i> Lease End Date</h4>
                    <p>Your lease agreement ends on this date.</p>
                </div>
            `;
        }
        
        // Bills
        if (dayEvents.bills.length > 0) {
            eventDetailsHTML += `<h4 class="event-section-title"><i class="fas fa-file-invoice-dollar"></i> Bills</h4>`;
            dayEvents.bills.forEach(bill => {
                const isOverdue = (bill.status === 'unpaid' && dateObj < new Date());
                eventDetailsHTML += `
                    <div class="event-detail bill-event ${isOverdue ? 'overdue' : bill.status}">
                        <h5>${bill.type.toUpperCase()} BILL - ₱${parseFloat(bill.amount).toFixed(2)}</h5>
                        <p>Status: <span class="status-badge ${bill.status}">${bill.status}</span></p>
                        ${bill.description ? `<p>${bill.description}</p>` : ''}
                        ${isOverdue ? '<p class="overdue-warning">This bill is overdue!</p>' : ''}
                    </div>
                `;
            });
        }
        ...
        // Payments
        if (dayEvents.payments.length > 0) {
            eventDetailsHTML += `<h4 class="event-section-title"><i class="fas fa-money-bill-wave"></i> Payments</h4>`;
            dayEvents.payments.forEach(payment => {
                eventDetailsHTML += `
                    <div class="event-detail payment-event ${payment.status}">
                        <h5>PAYMENT - ${payment.status.toUpperCase()}</h5>
                        <p>Status: <span class="status-badge ${payment.status}">${payment.status}</span></p>
                    </div>
                `;
            });
        }
        ...
        // Announcements
        if (dayEvents.announcements.length > 0) {
            eventDetailsHTML += `<h4 class="event-section-title"><i class="fas fa-bullhorn"></i> Announcements</h4>`;
            dayEvents.announcements.forEach(announcement => {
                eventDetailsHTML += `
                    <div class="event-detail announcement-event ${announcement.priority}">
                        <h5>${announcement.title} (${announcement.priority})</h5>
        ...
        // Maintenance
        if (dayEvents.maintenance.length > 0) {
            eventDetailsHTML += `<h4 class="event-section-title"><i class="fas fa-tools"></i> Maintenance</h4>`;
            dayEvents.maintenance.forEach(request => {
                eventDetailsHTML += `
                    <div class="event-detail maintenance-event ${request.status}">
                        <h5>MAINTENANCE REQUEST - ${request.status.replace('_', ' ').toUpperCase()}</h5>
        ...
        if (!dayEvents.leaseStart && !dayEvents.leaseEnd && dayEvents.bills.length === 0 && 
            dayEvents.announcements.length === 0 && dayEvents.maintenance.length === 0 && 
            dayEvents.payments.length === 0) {
            eventDetailsHTML += `
                <div class="no-events">
        ...
        eventDetailsHTML += `
                ...
                <div class="event-modal-footer">
                    <button onclick="closeEventModal()" class="btn-close">Close</button>
                </div>
            </div>
        ...
        modal.innerHTML = eventDetailsHTML;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Add click handler for calendar days (delegated event)
        document.addEventListener('click', function(e) {
            if (e.target.closest('.calendar-day.has-events')) {
                const dayElement = e.target.closest('.calendar-day');
                // Extract date from the element if needed
            }
        });
    });
</script>
