...
try {
    $query = "(SELECT 'bill' as type, 
                     CONCAT(
                         CASE 
                             WHEN bill_type = 'rent' THEN 'Rent Bill: ₱'
                             WHEN bill_type = 'utility' THEN 'Utility Bill: ₱'
                             WHEN bill_type = 'penalty' THEN 'Penalty: ₱'
                             WHEN bill_type = 'other' THEN 'Other Bill: ₱'
                             ELSE 'Bill: ₱'
                         END,
                         amount, ' - ', COALESCE(description, bill_type)
                     ) as message, 
                     due_date as date, 'medium' as priority
              FROM BILL b 
              JOIN LEASE l ON b.lease_id = l.lease_id
              WHERE b.status IN ('unpaid', 'overdue') AND l.tenant_id = $tenant_id)
              UNION ALL
        ...
        <div class="notifications-container">
            ...
            <?php else: ?>
                <?php foreach ($pageNotifications as $notification): ?>
                    <div class="page-notification <?= $notification['priority'] ?>">
