...
$last_viewed = $_SESSION['notifications_viewed_at'] ?? 0;

try {
    if (isset($conn)) {
        // Count bills created after last view
        $query = "SELECT COUNT(*) as count 
                 FROM BILL b 
                 JOIN LEASE l ON b.lease_id = l.lease_id 
                 WHERE l.tenant_id = ? AND UNIX_TIMESTAMP(b.created_at) > ?";
        
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ii", $tenant_id, $last_viewed);
        $stmt->execute();
        ...
        $query = "SELECT COUNT(*) as count 
                 FROM ANNOUNCEMENT 
                 WHERE visible_to IN ('tenant', 'all') AND UNIX_TIMESTAMP(created_at) > ?";
        
        $stmt = $conn->prepare($query);
        $stmt->bind_param("i", $last_viewed);
        $stmt->execute();
        $result = $stmt->get_result();
        $notification_count += $result->fetch_assoc()['count'];
...
function markNotificationsRead() {
    // Reset notification badge immediately
    ...
    fetch('', {
        ...
        body: 'mark_notifications_viewed=1'
    }).then(() => {
